...
def validate_c_identifier(name: str, context: str = "") -> None:
    ...
    if name in C_RESERVED_WORDS:
        ctx = f" ({context})" if context else ""
        raise ValidationError(
            f"Invalid C identifier '{name}'{ctx}. '{name}' is a C reserved word."
        )

    if len(name) > 63:
        # C99 requires at least 63 significant characters for identifiers
        ctx = f" ({context})" if context else ""
        print(f"Warning: Identifier '{name}'{ctx} is very long ({len(name)} chars). "
...
def validate_endpoint_ids(instance) -> List[str]:
    ...
    for ep in avlos_endpoints(instance):
        ...
        if ep_id in ep_id_map:
            errors.append(
                f"Duplicate endpoint ID {ep_id}: "
                f"'{ep.full_name}' and '{ep_id_map[ep_id].full_name}'"
            )
        else:
            ep_id_map[ep_id] = ep
